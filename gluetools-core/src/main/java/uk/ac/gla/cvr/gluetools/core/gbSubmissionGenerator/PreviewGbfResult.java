...
public class PreviewGbfResult extends MapResult {
	...
	public PreviewGbfResult(String sourceName, String sequenceID, byte[] gbfBytes) {
		super("previewGbfResult", mapBuilder()
				.put("source.name", sourceName)
				.put("sequenceID", sequenceID)
				.put("gbfString", new String(gbfBytes))
				);
	}

	@Override
	protected void renderToConsoleAsText(InteractiveCommandResultRenderingContext renderCtx) {
		renderCtx.output(getCommandDocument().getString("gbfString"));
	}
