...
public class RerootPhylogenyCommand extends BaseRerootCommand {
	...
	protected OkResult execute(CommandContext cmdContext, PhyloUtility phyloUtility) {
		...
		PhyloSubtree<?> outgroupSubtree;
		PhyloLeafFinder leafFinder = new PhyloLeafFinder(pLeaf -> pLeaf.getName().equals(outgroup));
		rerootedTree.accept(leafFinder);
		outgroupSubtree = leafFinder.getPhyloLeaf();
		
		if(outgroup != null && removeOutgroup) {
			removeOutgroupSubtree(rerootedTree, outgroupSubtree);
		}
