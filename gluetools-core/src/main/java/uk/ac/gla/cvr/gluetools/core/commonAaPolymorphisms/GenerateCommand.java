...
import uk.ac.gla.cvr.gluetools.core.command.project.alignment.AlignmentModeCommand.FeatureOfAncConstrainingRefCompleter;
...
@CommandClass(
		...
		description = "Generate common amino-acid polymorphism variations", 
		docoptUsages={"<alignmentName> -r <acRefName> -f <featureName> [-c] (-w <whereClause> | -a) [-p]"},
		docoptOptions={
			"-r <acRefName>, --acRefName <acRefName>        Ancestor-constraining ref",
			"-f <featureName>, --featureName <featureName>  Protein-coding feature",
			"-c, --recursive                                Include descendent members",
			"-p, --preview                                  Preview generated variations", 
			"-w <whereClause>, --whereClause <whereClause>  Qualify members",
		    "-a, --allMembers                               All members"},
		metaTags = {CmdMeta.updatesDatabase}, 
		furtherHelp = ""
)
public class GenerateCommand extends ModulePluginCommand<GenerateResult, CommonAaPolymorphismGenerator> implements ProvidedProjectModeCommand  {
	...
	@Override
	protected GenerateResult execute(CommandContext cmdContext, CommonAaPolymorphismGenerator generator) {
		Alignment alignment = GlueDataObject.lookup(cmdContext, Alignment.class, Alignment.pkMap(alignmentName));
		alignment.getAncConstrainingRef(cmdContext, acRefName);
		...
		List<AaPolymorphism> aaPolymorphisms = 
				generator.preview(cmdContext, alignment, ancConstrainingRef, featureName, whereClause, recursive);
		if(!preview) {
			generator.generate(cmdContext, aaPolymorphisms);
		}
		return new GenerateResult(aaPolymorphisms);
	}
