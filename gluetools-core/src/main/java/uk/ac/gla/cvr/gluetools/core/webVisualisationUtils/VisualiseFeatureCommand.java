...
@CommandClass(
		...
		metaTags = {
				// CmdMeta.webApiOnly, 
				CmdMeta.inputIsComplex}
)
public class VisualiseFeatureCommand extends ModulePluginCommand<PojoCommandResult<FeatureVisualisation>, WebVisualisationUtility> {
	...
	@Override
	protected PojoCommandResult<FeatureVisualisation> execute(CommandContext cmdContext, WebVisualisationUtility modulePlugin) {
		...
		// return new FeatureVisualisationResult(allColumnsAlmt.getSegments("query"), allColumnsAlmt.getSegments("reference"));
	...
	/*
	public static class FeatureVisualisationResult extends CommandResult {

		private FeatureVisualisationResult(List<QueryAlignedSegment> qaSegs, List<QueryAlignedSegment> refSegs) {
			super("featureVisualisationResult");
			CommandDocument commandDocument = super.getCommandDocument();
			CommandArray querySegsArray = commandDocument.setArray("querySegments");
			for(QueryAlignedSegment qaSeg: qaSegs) {
				CommandObject qaSegObj = querySegsArray.addObject();
				qaSeg.toDocument(qaSegObj);
			}
			CommandArray refSegsArray = commandDocument.setArray("refSegments");
			for(QueryAlignedSegment refSeg: refSegs) {
				CommandObject refSegObj = refSegsArray.addObject();
				refSeg.toDocument(refSegObj);
			}
