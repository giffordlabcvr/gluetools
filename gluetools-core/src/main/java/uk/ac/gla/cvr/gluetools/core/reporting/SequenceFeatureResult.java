...
public class SequenceFeatureResult {
	...
	public void init(
		...
		if(ntQueryAlignedSegments.isEmpty()) {
			...
			if(featureTreeResult.getFeatureMetatags().contains(FeatureMetatag.Type.OPEN_READING_FRAME.name())) {
				...
				if(orfAncestorFeatureName != null) {
					...
					List<AaReferenceSegment> ancestorAaRefSegs = featureToSequenceFeatureResult.get(orfAncestorFeatureName).aaQueryAlignedSegments;
					...
					if(codon1Start != null) {
						...
						if(ntOffset % 3 != 0) {
							throw new MutationFrequenciesException(MutationFrequenciesException.Code.FEATURE_CODON_NUMBERING_MISMATCH, 
									orfAncestorFeatureName, featureTreeResult.getFeatureName(), 
