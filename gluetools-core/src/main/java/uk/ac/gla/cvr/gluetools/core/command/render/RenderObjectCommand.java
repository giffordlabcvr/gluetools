...
@CommandClass(
		...
		docoptOptions = { },
		metaTags = { },
		furtherHelp = "The supplied <rendererModuleName> refers to a module implementing the ObjectRenderer interface."
)
public class RenderObjectCommand extends Command<CommandResult> {
	...
	public void configure(PluginConfigContext pluginConfigContext, Element configElem) {
		...
		rendererModuleName = PluginUtils.configureStringProperty(configElem, RENDERER_MODULE_NAME, true);
	}

	
	@Override
	public CommandResult execute(CommandContext cmdContext) {
		ObjectRenderer<?> renderer = ObjectRenderer.getRenderer(cmdContext, rendererModuleName);
		RenderableObjectMode renderableObjectMode = (RenderableObjectMode) cmdContext.peekCommandMode();
		return renderer.render(cmdContext, renderableObjectMode.getRenderableObject(cmdContext));
	}
